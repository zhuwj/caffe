...
void fillCropSize(int input_height, int input_width,
    ...
    for (int h = 0; h < scale_rates.size(); ++h){
      ...
      crop_h = (abs(crop_h - net_input_height) < 3)?net_input_height:crop_h; //why do this step?
      for (int w = 0; w < scale_rates.size(); ++w){
        ...
        if (abs(h-w)<=max_distort) { //height ratio and width ratio could not diff too much, i.e., choose a near-diagnal band area in the combination matrix
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
                                       Dtype* transformed_data, const int chn_flow_single) {
  ...
  if (param_.is_flow())
  {
    ...
    CHECK_GE(datum_channels % (chn_flow_single * 2), 0);
  }
  ...
  for (int c = 0; c < datum_channels; ++c) {
    ...
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        if (need_imgproc){
          if (has_uint8){
        	  if (param_.is_flow() && do_mirror && is_flow_x)
